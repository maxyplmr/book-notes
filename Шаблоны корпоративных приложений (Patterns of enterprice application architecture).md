...
# Организация бизнес логики / предметной области
...
# Сеансы и состояния / Стратегии распределенных вычислений

### Способы сохранения состояния сеанса
 - Сохранение на стороне клиента ( Client session state )
 - Сохранение на стороне сервера ( Server session state ) 
 - Сохранение в БД ( Database session state ) 

### Сужение границ распределения 
 - Интерфейс удаленного доступа ( Remote Facade )
 - Обьект переноса данных ( Data Transfer Object )
...
### Модель Нильссона  
 - Потребитель 
 - Вспомогательный слой потребителя
 - Приложение 
 - Домен
 - Доступ к хранимым данным
 - Общедоступные хранимые процедуры
 - Приватные хранимые процедуры
 
 `Часть 2 - Детали`
 
**Сценарий транзакций ( Transaction Script )** - способ организации бизнес логики по процедурам,
каждая из которых обслуживает один запрос инициируемый слоем представления.
- Простота
- Эффективная производительность
- Для небольших приложений
- Дублирование кода, потому что цель (обслуживать одну транзакцию)

**Модель предметной области ( Domain Model )** - объектная модель домена, поведение (функции) и свойства (данные)
- Виды: Простая и Сложная 
- Максимум снизить зависимости от других слоев системы
- Рекомендовано размещать родственный код в приделах одного класса, разделение по целесообразности.
- Взаимодействие с БД через - преобразователь данных
- Для API - слой служб

**Модуль таблицы ( Table Model )** - обьект, охватывающий логику обработки всех записей таблицы БД
 - По одному классу на каждую таблицу
 - Единственный экземпляр класса содержит всю логику обработки данных таблицы
 - Может содержать методы-оболочки предоставляющие запросы к БД
 - Ведет себя лучше чем связка (модель предметной области и активная запись)

**Слой служб ( Service Layer )** - схема определения границ приложения, устанавливает множество доступных действий
 - Пользовательские интерфейсы || Слой служб || Модель домена || Источник данных
 - Инкапсуляция бизнес логики 
 - Координация транзакциями 
 - Представляет типовое решение организации бизнес логики - логика домена / логика приложения или логика рабочего процесса 
 - Варианты реализации слоя - интерфейс доступа к домену / сценарий операции
